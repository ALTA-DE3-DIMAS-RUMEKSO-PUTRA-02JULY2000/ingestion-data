...
class Extraction():
    ...
    def __read_csv(self) -> pd.DataFrame:
        """
        problem: DtypeWarning: Columns (6) have mixed types.
    ...
    def __read_json_chunked(self) -> None:
        ...
        For HTTP(S) URLs the key-value pairs are forwarded to urllib.request.
        Request as header options.

        """

        chunk_size = 50000
        with pd.read_json(self.url, lines=True, storage_options=storage_options, chunksize=chunk_size, compression="gzip") as reader: # noqa
            for chunk in reader:
                self.dataframe = pd.concat([self.dataframe, chunk], ignore_index=True) # noqa
    ...
    def cast_data(self):
        if self.extension == "json":
            ...
            self.dataframe["public"] = self.dataframe["public"].astype("string") # noqa
            self.dataframe["created_at"] = pd.to_datetime(self.dataframe["created_at"]) # noqa
        else:
...
class Load():
    ...
    def __create_connection(self):
        ...
        database = "mydb"
        port = 5432
        conn_string = f"postgresql://{user}:{password}@{host}:{port}/{database}" # noqa

        self.engine = create_engine(conn_string)

    def to_postgres(self, db_name: str, data: pd.DataFrame):
        from sqlalchemy.types import BigInteger, String, JSON, DateTime, Boolean # noqa
        ...
        try:
            ...
            data.to_sql(name=self.db_name, con=self.engine, if_exists="replace", index=False, schema="public", dtype=df_schema, method=None, chunksize=5000) # noqa
...
def main():
    ...
    # url = f"http://data.gharchive.org/{year}-{month:02}-{day:02}-{hour}.json.gz" # noqa
    # print("url: ", url)
